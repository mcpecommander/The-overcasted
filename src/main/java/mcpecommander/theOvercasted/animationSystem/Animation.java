...
public class Animation {
	 ...
	 * 
	 * @param fps
	 *            FPS to have this run at.
	 * @param startingFrame
	 *            In case the animation start from a different frame other than 0.
	 * @param model
	 *            The model wished to apply the animation to.
	 */
	public void playAnimation(int fps, int startingFrame, CraftStudioModelSon model) {
		if (!hasFinished && !Minecraft.getMinecraft().isGamePaused()) {
			if (firstFrame) {
			...
			for (ImmutablePair<String, SortedMap<Integer, KeyFrame>> animation : animations) {
				...
				if (box != null) {
					...
					if (frame == null) {
						...
						// Does not really support multiple rotations on the same box from different
						// animations.
						box.addTranslate(frame.getTranslate());
						// Do not have access to the default stretch.
						box.resetStretch();
						box.setStretch(box.getStretchAsVector().x + frame.getScale().x,
								box.getStretchAsVector().y + frame.getScale().y,
								box.getStretchAsVector().z + frame.getScale().z);
	...
	/**
	 * A method to return a fully interpolated sub-animation from the given key
	 * frames.
	 * 
	 * @param length
	 *            The animation length, note that it should be 1 frame larger than
	 *            the last frame to insure a smooth lerp.
	 * @param box
	 *            The name of the modelRenderer that this sub-animation belongs to.
	 * @param frames
	 *            The frames
	 * @return An {@link ImmutablePair} with a the box name in the left and the
	 *         interpolated animation in the right.
	 */
	@Nullable
				...
				if (i <= currentFrame) {
					...
					if (it.hasNext()) {
						...
						for (int j = currentFrame + 1; j < length; j++) {
							...
							Vector3f interpolatedScale = new Vector3f();
							interpolatedScale.x = lerp(animation.get(currentFrame).getScale().x, lastFrame.getScale().x,
									t);
							interpolatedScale.y = lerp(animation.get(currentFrame).getScale().y, lastFrame.getScale().y,
									t);
							interpolatedScale.z = lerp(animation.get(currentFrame).getScale().z, lastFrame.getScale().z,
									t);
							Vector3f interpolatedTranslate = new Vector3f();
							interpolatedTranslate.x = lerp(animation.get(currentFrame).getTranslate().x,
							...
							Vector3f interpolatedRotation = new Vector3f();
							interpolatedRotation.x = rotationalInterpolation(
									animation.get(currentFrame).getRotation().x, lastFrame.getRotation().x, t);
							interpolatedRotation.y = rotationalInterpolation(
									animation.get(currentFrame).getRotation().y, lastFrame.getRotation().y, t);
							interpolatedRotation.z = rotationalInterpolation(
									animation.get(currentFrame).getRotation().z, lastFrame.getRotation().z, t);
							KeyFrame interpolatedFrame = new KeyFrame(interpolatedScale, interpolatedRotation,
									interpolatedTranslate, j);
							temp.put(j, interpolatedFrame);
						}
	 ...
	 * 
	 * @param location
	 *            The animation file name with the extension {@code .csjsmodelanim}
	 *            and the full location.
	 ...
	 * 
	 * @param entry
	 *            The json object entries.
	 * @param animLength
	 *            The animation length and not just the length of this
	 *            sun-animation.
	 * @return A {@link ImmutablePair} that includes the full interpolated
	 *         sub-animation for one block.
	 */
	private static ImmutablePair<String, SortedMap<Integer, KeyFrame>> readAnimBlock(Entry<String, JsonElement> entry,
			int animLength) {
	 ...
	 * 
	 * @param objR
	 *            The json rotation array.
	 * @param objP
	 *            The json translate array.
	 * @param objS
	 *            The json scale array.
	 * @return An array of key frames including no repeated key frames where a two
	 *         key frames have the same number.
	 */
	private static KeyFrame[] AddFrames(JsonObject objR, JsonObject objP, JsonObject objS) {
		...
		for (Entry<String, JsonElement> entry : objR.entrySet()) {
			...
			frame.rotation = new Vector3f(array.get(0).getAsFloat(), -array.get(1).getAsFloat(),
		...
		for (Entry<String, JsonElement> entry : objP.entrySet()) {
			...
			if (frames.contains(frame)) {
				frames.get(frames.indexOf(frame)).translate = new Vector3f(array.get(0).getAsFloat(),
						-array.get(1).getAsFloat(), -array.get(2).getAsFloat());
			} else {
				frame.translate = new Vector3f(array.get(0).getAsFloat(), -array.get(1).getAsFloat(),
						-array.get(2).getAsFloat());
				frames.add(frame);
			}
		...
		for (Entry<String, JsonElement> entry : objS.entrySet()) {
			...
			array = entry.getValue().getAsJsonArray();
			if (frames.contains(frame)) {
				frames.get(frames.indexOf(frame)).scale = new Vector3f(array.get(0).getAsFloat(),
						array.get(1).getAsFloat(), array.get(2).getAsFloat());
			} else {
				frame.scale = new Vector3f(array.get(0).getAsFloat(), array.get(1).getAsFloat(),
						array.get(2).getAsFloat());
				frames.add(frame);
			}
