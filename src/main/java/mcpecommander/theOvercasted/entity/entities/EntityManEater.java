...
public class EntityManEater extends EntityMob implements IAnimated{
	...
	public void onLivingUpdate() {
		...
		if(this.isBeingRidden() && this.getPassengers().get(0) instanceof EntityPlayer) {
			...
			if(this.ticksExisted%3 == 0) this.attackEntityAsMob(player);
	...
	//We increase the size to make it easier to attack the mob when being trapped so we adjust the mounting height too.
	@Override
	...
	//The fucking canCollide boolean does not stop collision so we override this and add our trapping code.
	@Override
	protected void collideWithEntity(Entity entityIn) {
		if(entityIn instanceof EntityPlayer && !this.isBeingRidden() && !((EntityPlayer) entityIn).capabilities.isCreativeMode && this.deathTime < 1) {
